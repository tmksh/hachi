...
function mapItems(d: Detail): LineItem[] {
  return (d.items ?? []).map((it: { description: string; quantity: number | string; unit_price: number | string }) => ({
...
export function InvoiceDetailClient({ initialData }: InvoiceDetailClientProps) {
  const { id } = useParams();
    ...
    <div className="p-4 md:p-6 space-y-4 print:p-0 print:space-y-3">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 print:hidden">
          ...
          {editing ? (
            <>
              <Button variant="outline" size="sm" onClick={() => { setEditing(false); resetForm(data); }}>
