...
export function CrmClient({
  ...
  const { data, isLoading: loadingCustomers } = useCustomers(
    page,
    debouncedSearch,
    !isUnfollowedTab,
    initialCustomers,
  ...
  const { data: counts } = useCustomerCounts(initialCounts);
  const { data: unfollowedCount = initialUnfollowedCountProp } = useUnfollowedCustomersCount(initialUnfollowedCountProp);
  ...
  const { data: dealSummaries } = useCustomerDealSummaries(
    customerIds,
    page === 1 && !debouncedSearch && !isUnfollowedTab ? initialDealSummaries : undefined,
  );
