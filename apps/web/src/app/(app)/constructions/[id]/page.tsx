...
export default function ConstructionDetailPage() {
  ...
  useEffect(() => {
    if (id) getConstruction(id as string).then(setData).catch(() => {}).finally(() => setLoading(false));
  }, [id]);
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-semibold">{data.construction_no}</h1>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card><CardHeader className="pb-3"><CardTitle className="text-sm">金額</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">
              ...
              {data.status === "completed" && data.order_amount && data.actual_cost ? (
                <div className="flex justify-between border-t pt-2 mt-2">
                  <span className="text-muted-foreground font-medium">確定粗利</span>
                  <span className={`font-semibold tabular-nums ${data.order_amount - data.actual_cost >= 0 ? "text-green-700" : "text-red-600"}`}>
                    ¥{(data.order_amount - data.actual_cost).toLocaleString()}
                    <span className="text-xs ml-1 font-normal">
                      ({((data.order_amount - data.actual_cost) / data.order_amount * 100).toFixed(1)}%)
                    </span>
                  </span>
                </div>
              ) : null}
