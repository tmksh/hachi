...
function mapEstimateItems(est: EstimateDetail): LineItem[] {
...
type QuoteEditClientProps = {
  initialEstimate: EstimateDetail | null;
  initialCustomers: { id: string; name: string }[];
};

export function QuoteEditClient({ initialEstimate, initialCustomers }: QuoteEditClientProps) {
  const { id } = useParams();
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [customers] = useState(initialCustomers);
  const [customerId, setCustomerId] = useState(initialEstimate?.customer_id ?? "");
  const [title, setTitle] = useState(initialEstimate?.title ?? "");
  const [notes, setNotes] = useState(initialEstimate?.notes ?? "");
  const [status, setStatus] = useState<string>(initialEstimate?.status ?? "draft");
  const [items, setItems] = useState<LineItem[]>(() =>
    initialEstimate ? mapEstimateItems(initialEstimate) : DEFAULT_ITEMS
  );

  if (!initialEstimate) {
    return (
      <div className="p-4 md:p-6 space-y-4">
        <Link href="/quotes" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
