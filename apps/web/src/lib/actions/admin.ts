...
export async function getAdminUsageSummary(): Promise<{
  companyUsage: AdminCompanyUsage[];
  aiUsage: AdminAiUsageSummary;
}> {
  const supabase = await assertSuperAdmin();
  const { data, error } = await supabase.rpc("admin_usage_summary");
  if (error || !data) throw new Error("利用状況を取得できませんでした。DBの更新状態を確認してください。");
  return data as { companyUsage: AdminCompanyUsage[]; aiUsage: AdminAiUsageSummary };
}
