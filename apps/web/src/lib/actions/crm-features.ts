...
export async function sendSchedulingEmail(input: {
  ...
  try {
    ...
    const { sendResendEmail, CUSTOMER_FROM_EMAIL, textToEmailHtml } = await import("@/lib/resend");
    ...
    if (mailError) {
      throw new Error(
        `メール送信に失敗しました: ${mailError}（Gmail未連携のため Resend 経由。届かない場合はメール設定でGmail連携してください）`,
      );
    }
  }
